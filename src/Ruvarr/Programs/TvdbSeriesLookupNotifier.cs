...
public sealed class TvdbSeriesLookupNotifier
{
    private readonly Lock _lock = new();
    private readonly Dictionary<int, TvdbSeriesLookupQueueItemSummary> _items = [];
    ...
    public void Enqueue(int ruvId, string programName)
    {
        lock (_lock)
        {
            ...
            _items[ruvId] = new TvdbSeriesLookupQueueItemSummary(ruvId, programName, TvdbSeriesLookupStatus.Pending);
        }
    ...
    public void MarkProcessing(int ruvId)
    {
        lock (_lock)
        {
            if (_items.TryGetValue(ruvId, out TvdbSeriesLookupQueueItemSummary? item))
            {
                _items[ruvId] = item with { Status = TvdbSeriesLookupStatus.Processing };
            }
        }
    ...
    public IReadOnlyList<TvdbSeriesLookupQueueItemSummary> Items
    {
        get
        {
            lock (_lock)
